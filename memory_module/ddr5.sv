...
module ddr5 (
    ...
    reg [13:0]   ca_raw_r[1:0];   // Transmitted CA before CAI restoration
    reg          cai_r   [1:0];   // Command/address inversion flag per channel
...
//restore cai
    always @(dram_if.ca or dram_if.cai) begin
        ca_raw_r[0] <= #BUS_DELAY dram_if.ca[13:0];
        ca_raw_r[1] <= #BUS_DELAY dram_if.ca[27:14];
        cai_r[0]    <= #BUS_DELAY dram_if.cai[0];
        cai_r[1]    <= #BUS_DELAY dram_if.cai[1];
        ca_r[0]     <= #BUS_DELAY ((dram_if.cai[0] === 1'b1) ? ~dram_if.ca[13:0]  : dram_if.ca[13:0]);
        ca_r[1]     <= #BUS_DELAY ((dram_if.cai[1] === 1'b1) ? ~dram_if.ca[27:14] : dram_if.ca[27:14]);
    end
    ...
    always @(dram_if.dqs_t or dram_if.dqs_c) begin
        ...
        dqs_r[0] <= #BUS_DELAY {dram_if.dqs_c[`DQS_PER_CH-1:0], dram_if.dqs_t[`DQS_PER_CH-1:0]};
         // Channel 1: DQS_T[7:4], DQS_C[7:4]
        dqs_r[1] <= #BUS_DELAY{dram_if.dqs_c[DQS_BITS-1:`DQS_PER_CH], dram_if.dqs_t[DQS_BITS-1:`DQS_PER_CH]};
    ...
    // =========================================================================
    function automatic [3:0] count_ca_ones;
        input [13:0] ca_word;
        integer b;
        begin
            count_ca_ones = '0;
            for (b = 0; b < 14; b = b + 1) begin
                if (ca_word[b] === 1'b1)
                    count_ca_ones = count_ca_ones + 1'b1;
            end
        end
    endfunction
...
//CS has remained HIGH for 40ns
    ...
    task cmd_task;
        ...
        //---------------------------------------------------   
	    if (c_csn && !(act_c1[ch] || wr_c1[ch] || rd_c1[ch] || mrw_c1[ch])) begin
                cmd_state[ch] = CMD_ST_IDLE; // idel sate 
                // CS_n=H → DES/NOP
            end else begin
        //----------------------------------------------------
            if (c_cai) begin
                $display("[CH%0d][CAI DRAM] t=%0t transmitted_ca=%014b CAI=1 restored_ca=%014b ones=%0d",
                         ch, $time, c_ca_raw, c_ca, count_ca_ones(c_ca));
            end
            if (!reset_init_done[ch]) begin
...
//
       ...
       //                        C Y C L E - T W O 
		...
		bank      = wr_bank[ch];
                col = {c_ca[7], c_ca[6], c_ca[5], c_ca[4],
		  ...
		  //  This block pushes the WRITE command into a delayed pipeline queue (wr_pipe) along with its bank, row, column, and burst length so that the actual memory write occurs after the configured write latency (write_latency) cycles.
            ...
            // FSM state: READ (cycle 2)
                ...
                bank      = rd_bank[ch];
                col = {c_ca[7], c_ca[6], c_ca[5], c_ca[4],
                       c_ca[3], c_ca[2], c_ca[1], c_ca[0], 2'b00};
                if (!init_done)
...
// Write burst activation
            ...
            dq_oe_d  [ch] <= #(odly) {`DQ_PER_CH {dq_oe  [ch]}};
            dq_out_d [ch] <= #(odly) {`DQ_PER_CH {dq_out [ch]}};
        end
    endtask
    ...
    always @(negedge rst_n_r or posedge diff_ck[0] or negedge diff_ck[0]) begin : main_ch0
        ...
        if (!rst_n_r) begin
            ...
            if (diff_ck[0]) begin
                ...
                tm_ck_pos[0] = $time;
                cmd_task(0, prev_cke[0], cke_r[0], csn_r[0], ca_r[0], cai_r[0], ca_raw_r[0]);
    ...
    always @(negedge rst_n_r or posedge diff_ck[1] or negedge diff_ck[1]) begin : main_ch1
        ...
        if (!rst_n_r) begin
            ...
            data_task(1);
                ...
                tm_ck_pos[1] = $time;
                cmd_task(1, prev_cke[1], cke_r[1], csn_r[1], ca_r[1], cai_r[1], ca_raw_r[1]);
