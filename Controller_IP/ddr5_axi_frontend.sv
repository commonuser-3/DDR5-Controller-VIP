...
//
// Responsibilities:
//   • Accept AXI4 AW/W/AR transactions
//   • Decode byte address into {BG, BA, ROW, COL, SUB_CH}
//   • Issue single-beat requests (BL16 fixed; AXI len=0)
//   • Buffer write data and return read data on R channel
//   • Generate write-response (B channel)
//
// Address mapping (byte address → DRAM fields):
//   addr[3:0]   = burst-internal byte offset (not sent to DRAM)
//   addr[5:4]   = BL16 column offset         (not sent to DRAM)
//   addr[14:6]  = col[8:0]  (COL[9] driven to 0, BL16 aligned)
//   addr[16:15] = ba[1:0]
//   addr[18:17] = bg[1:0]
//   addr[34:19] = row[15:0]
//   addr[35]    = rank (tied to 0 for single-rank)
// =============================================================
...
/*module ddr5_axi_frontend (
  ...
  // -----------------------------------------------------------
  // Write path: collect awlen+1 64-bit beats, then send one wide request.
  // -----------------------------------------------------------
  logic                   aw_held;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (s_wvalid && s_wready) begin
        wr_data_packed[wr_beat_cnt*64 +: 64] <= s_wdata;
        wr_strb_packed[wr_beat_cnt*8 +: 8] <= s_wstrb;
        if ((wr_beat_cnt == aw_len_r[2:0]) || s_wlast || (wr_beat_cnt == 3'd7)) begin 
          wr_req_pending <= 1'b1;
          aw_held <= 1'b0;
        end else begin
  ...
  // -----------------------------------------------------------
  // Write response (B channel): registered 1 cycle after req accepted
  // -----------------------------------------------------------
  logic [3:0] b_id_r;
  logic                 b_pending;
  ...
  assign s_rdata  = r_data_r[r_beat_cnt*64 +: 64];
  assign s_rid    = r_id_r;
  assign s_rresp  = 2'b00;
  assign s_rlast  = (r_beat_cnt == r_len_r[2:0]) || (r_beat_cnt == 3'd7);
